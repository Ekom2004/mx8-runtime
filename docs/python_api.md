...
`MX8_VIDEO_EXPERIMENTAL_DIRECT_DECODE_TO_DESTINATION=1` (requires direct-write mode + `nvdec|auto`) enables an internal decode-to-destination attempt: MX8 tries per-clip NVDEC decode from local media paths and writes each decoded clip into the Torch-owned CUDA destination on the current stream, then fails open to the existing payload direct-write/staged-copy path if unsupported.
