...
When coordinator leader fencing is enabled (default), mutating RPCs (`RegisterNode`, `Heartbeat`, `RequestLease`, `ReportProgress`) return `FAILED_PRECONDITION` on followers or stale leaders with a `not leader for mutating operation` message.
