...
#[pyclass]
struct AudioLoader {
    loader: AudioLoaderInner,
    sample_count: usize,
    channels: usize,
    sample_rate_hz: Option<u32>,
    decode_error_policy: AudioDecodeErrorPolicy,
    decoded_samples_total: Arc<AtomicU64>,
    decode_failures_total: Arc<AtomicU64>,
    decoded_frames_total: Arc<AtomicU64>,
}
...
fn pcm_int_to_f32(sample: i64, bits_per_sample: u32) -> Result<f32> {
...
fn downmix_interleaved_to_mono(interleaved: &[f32], channels: usize) -> Result<Vec<f32>> {
    if channels == 0 {
...
fn decode_wav_audio(bytes: &[u8]) -> Result<(Vec<f32>, u32)> {
...
fn decode_flac_audio(bytes: &[u8]) -> Result<(Vec<f32>, u32)> {
    let cursor = std::io::Cursor::new(bytes);
...
fn decode_audio_mono_f32(bytes: &[u8]) -> Result<(Vec<f32>, u32)> {
