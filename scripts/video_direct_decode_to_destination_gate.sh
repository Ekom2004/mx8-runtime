...
# Video direct-decode-to-destination gate (GPU):
# - builds and loads mx8_video custom op
# - enables nvdec + direct-write + direct-decode-to-destination
# - asserts activation/fallback counters through python gate
#
# Usage:
#   ./scripts/video_direct_decode_to_destination_gate.sh
...
if ! command -v "${MX8_NVIDIA_SMI_BIN:-nvidia-smi}" >/dev/null 2>&1; then
  echo "[mx8] nvidia-smi not found on PATH (MX8_NVIDIA_SMI_BIN=${MX8_NVIDIA_SMI_BIN:-nvidia-smi})" >&2
  exit 1
fi

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/mx8-video-direct-decode-gate-XXXXXX")"
...
echo "[mx8] python video device-output gate (direct-decode-to-destination enabled)"
MX8_VIDEO_DEVICE_OUTPUT_TMP_ROOT="${TMP_ROOT}" \
MX8_VIDEO_DECODE_BACKEND=nvdec \
MX8_VIDEO_EXPERIMENTAL_DIRECT_DECODE_TO_DESTINATION=1 \
MX8_VIDEO_DIRECT_WRITE_OP_LIBRARY="${OP_LIB_PATH}" \
  "${PYTHON_BIN}" "${ROOT}/crates/mx8-py/python/m8_video_device_output_gate.py"

echo "[mx8] video_direct_decode_to_destination_gate OK"
