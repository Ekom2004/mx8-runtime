...
# Video direct-decode stress gate (GPU):
# - builds/loads mx8_video custom op
# - compares direct-decode path to baseline for payload parity
# - validates stride/stream invariants under alternating CUDA streams
#
# Usage:
#   ./scripts/video_direct_decode_stress_gate.sh
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/mx8-video-direct-decode-stress-gate-XXXXXX")"
...
echo "[mx8] python direct-decode stress gate"
MX8_VIDEO_DEVICE_OUTPUT_TMP_ROOT="${TMP_ROOT}" \
MX8_VIDEO_DIRECT_WRITE_OP_LIBRARY="${OP_LIB_PATH}" \
  "${PYTHON_BIN}" "${ROOT}/crates/mx8-py/python/m8_video_direct_decode_stress_gate.py"

echo "[mx8] video_direct_decode_stress_gate OK"
