...
# Multi-rank mix gate:
# - deterministic replay for same seed/epoch
# - no overlap across ranks
# - epoch drift digest change
#
# Usage:
#   ./scripts/mix_multirank_gate.sh
#
# Optional:
#   WORLD_SIZE=4 MX8_MIX_DDP_STEPS=128 ./scripts/mix_multirank_gate.sh
...
echo "[mx8] mix multirank gate: cargo test -p mx8-py mix_"
CARGO_INCREMENTAL=0 RUSTFLAGS="-C debuginfo=0" cargo test -p mx8-py mix_

TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/mx8-mix-multirank-gate-XXXXXX")"
...
echo "[mx8] python mix multirank gate"
...
MX8_MIX_DDP_STEPS="${MX8_MIX_DDP_STEPS:-128}" \
  "${PYTHON_BIN}" "${ROOT}/crates/mx8-py/python/m7_mix_multirank_gate.py"

echo "[mx8] mix_multirank_gate OK"
