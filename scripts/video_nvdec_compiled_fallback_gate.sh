...
# NVDEC compiled-path fallback gate:
# - build with `mx8_video_nvdec` cfg enabled
# - request `MX8_VIDEO_DECODE_BACKEND=nvdec`
# - verify loader still delivers batches via fail-open fallback when CUDA/NVIDIA is unavailable
#
# Usage:
#   ./scripts/video_nvdec_compiled_fallback_gate.sh
...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/mx8-video-nvdec-compiled-fallback-gate-XXXXXX")"
